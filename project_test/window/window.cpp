    ...
    switch (msg) {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == 1) {
                ...
                std::string messageText = "Name: " + std::string(name) + "\nUsername: " + std::string(username);
                MessageBox(hwnd, messageText.c_str(), TEXT("Result"), MB_OK);
            }
            break;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd = CreateWindow(wc.lpszClassName, TEXT("Name and Username"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 350, 150, NULL, NULL, hInstance, NULL);
